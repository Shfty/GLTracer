...
Grid::Grid( std::vector< Primitive* > primitives, glm::vec3 p0, glm::vec3 p1 )
{
    m_arrayLength = GRID_SIZE * GRID_SIZE * GRID_SIZE;
    m_grid = new int[ m_arrayLength ];
    m_p0 = p0;
    m_p1 = p1;

    m_cellSize = ( m_p1 - m_p0 ) / float( GRID_SIZE );
                ...
                for( int i = 0; i < primitives.size(); ++i )
                {
                    ...
                    switch( primitives[ i ]->Type )
                    {
                        ...
                        */
                        ...
                        case Primitive::AABB:
                        {
                            if( Collisions::AABBContainsAABB( pos0, pos1, op0, op1 ) )
                            {
                                m_cellObjectRefs.push_back( primitives[ i ]->ID );
                                objectCell = true;
                            }
