...
glm::vec2 renderBounds = windowBounds;//glm::vec2( 640.0f, 480.0f );
...
GLTracer::GLTracer()
{
    ...
    m_grid = new Grid( scene->GetObjects(), glm::vec3( -100, -100, -100 ), glm::vec3( 100, 100, 100 ) );
#endif
    ...
    glUniform1i( glGetUniformLocation( m_raytracerProgram, "PrimitiveSampler" ), 1 );
    glUniform1i( glGetUniformLocation( m_raytracerProgram, "AccellStructureSampler" ), 2 );
    glUniform1i( glGetUniformLocation( m_raytracerProgram, "ObjectRefSampler" ), 3 );
}
...
void GLTracer::Draw()
{
    ...
    glLoadIdentity();
    gluPerspective( FOV, renderBounds.x / renderBounds.y, 0.0f, 10.0f );
...
void GLTracer::initGL()
{
    ...
    if( !glfwOpenWindow( windowBounds.x, windowBounds.y, 0, 0, 0, 0, 0, 0, GLFW_WINDOW ) )
    {
        glfwTerminate();
        exit( -1 );
    }
    ...
    // Setup window
    glfwSetWindowPos( vm.Width / 2 - windowBounds.x / 2, vm.Height / 2 - windowBounds.y / 2 );
    glfwSetWindowTitle( windowTitle.c_str() );
    glfwSetWindowSizeCallback( callbackResizeWindow );
    glfwSetWindowCloseCallback( callbackCloseWindow );
    glfwSetMousePosCallback( Controls::UpdateMousePos );
...
void GLTracer::setupRenderTexture()
{
    ...
    glTexImage2D( GL_TEXTURE_2D, 0, GL_RGB, renderBounds.x, renderBounds.y, 0, GL_RGB, GL_UNSIGNED_BYTE, 0 );
    ...
    glFramebufferTexture( GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, m_screenTexture, 0 );
...
void GLTracer::setupShaders()
{
    m_basicVS = new ShaderProgram( std::string("BasicVert.vert"), GL_VERTEX_SHADER );
    m_basicFS = new ShaderProgram( std::string("BasicFrag.frag"), GL_FRAGMENT_SHADER );
    m_raytracerFS = new ShaderProgram( std::string("Raytracer.frag"), GL_FRAGMENT_SHADER );
    ...
    std::cout << std::endl << "Program info log:" << std::endl << raytracerInfoLog << std::endl;
    ...
    std::cout << std::endl << "Program info log:" << std::endl << basicInfoLog << std::endl;
}
...
void GLTracer::callbackResizeWindow( const int width, const int height )
{
...
// Updates the OpenGL viewport size and dependant variables
int GLTracer::callbackCloseWindow( void )
{
