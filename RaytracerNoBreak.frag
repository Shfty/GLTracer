...
// Returns 1.0 if pt is contained within a unit cube centered at the origin
    ...
    )
{
    ...
    float d = dot( pp, pp );
    hit = mix( hit, 0.0, step( 1.0, d ) ); // d > 1.0
...
/*
 * BEGIN EXPERIMENTAL BVH TRAVERSAL
 */
        // Initialization
               ...
               cell.z >= 0 && cell.z <= GRID_SIZE )
        {
            ...
            int objectRefCell = int( texelFetch( AccellStructureSampler, idx ).r );

            if( objectRefCell != -1 )
            {
                int objectRefContents = int( texelFetch( CellObjectSampler, objectRefCell ).r );
                ...
                while( true )
                {
                    ...
                    objectRefCell++;
                    objectRefContents = int( texelFetch( CellObjectSampler, objectRefCell ).x );
                }
            }
...
/*
 * END EXPERIMENTAL BVH TRAVERSAL
 */

/* Brute force object traversal
